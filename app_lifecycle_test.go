...
func TestStartClosesResourcesAfterModuleInitFailure(t *testing.T) {
	...
	app := New(WithRouter(&noopRouter{}), WithLogger(discardLogger{}))
...
func TestStartClosesResourcesAfterGRPCListenFailure(t *testing.T) {
	...
	var events []string
	module := &lifecycleGRPCModule{
		lifecycleModule: &lifecycleModule{name: "user", events: &events},
	}
	app := New(
		WithAddr("127.0.0.1:0"),
		WithGRPCAddr(listener.Addr().String()),
